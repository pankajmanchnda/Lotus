...
export default function App() {
  const [intake, setIntake] = useState<UserIntake>({
    ...
    dailySteps: 6200,
    selectedSymptoms: [],
    symptomText: "thyroid, blood pressure, low energy",
    goalsText: "get fit and agile, get active",
    allergies: []
  });
          ...
          <div className="space-y-5">
            ...
            <div>
              ...
              <textarea
                ...
                placeholder="e.g., thyroid, blood pressure, diabetes, fatigue, cholesterol, sleep issues"
                value={intake.symptomText}
                onChange={(event) => setIntake({ ...intake, symptomText: event.target.value })}
              />
              <p className="text-[10px] text-slate-500 mt-1">Free-text conditions are matched against the family protocol PDF where available.</p>
            </div>
            ...
            <div>
              ...
              <textarea
                className="w-full min-h-20 bg-slate-800 border border-slate-700 rounded p-2 text-sm text-slate-100 focus:outline-none focus:border-emerald-500"
                placeholder="e.g., get fit and agile, get thin, get bulky, get active, get glowing skin"
                value={intake.goalsText}
                onChange={(event) => setIntake({ ...intake, goalsText: event.target.value })}
              />
              <p className="text-[10px] text-slate-500 mt-1">If a goal is not in the PDF, the app will avoid inventing a medicine protocol.</p>
            </div>
              ...
              <div className="bg-slate-900 border border-slate-800 rounded-xl p-6 shadow-xl">
                ...
                {result.protocolMatches.length === 0 ? (
                  <p className="text-sm text-slate-500 mt-4 italic">
                    No direct protocol match was found in the attached family PDF for these free-text symptoms or goals. This is intentional: the engine will not invent medicines for unmatched goals such as diabetes, bulky muscle gain, or glowing skin until a source-backed protocol is added.
                  </p>
                ) : (
                    ...
                    {result.protocolMatches.map((protocol) => (
                      <div key={protocol.id} className="bg-slate-950 border border-slate-800 rounded-lg p-4">
                        <div className="flex flex-wrap items-center justify-between gap-3 mb-3">
                          ...
                          <span className="text-[10px] uppercase tracking-widest text-slate-400 border border-slate-700 rounded px-2 py-1">
                            {protocol.sourceDocument}
                          </span>
                        </div>
                        <p className="text-xs text-slate-300 mb-3">{protocol.objective}</p>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-xs">
                          <div>
                            <span className="block font-bold text-slate-400 mb-1">Medicines From PDF</span>
                            <ul className="space-y-1 bg-slate-900/60 p-2 rounded border border-slate-800/80 text-slate-300">
                          ...
                          <div>
                            <span className="block font-bold text-slate-400 mb-1">Timing / Dosage</span>
                            <p className="bg-slate-900/60 p-2 rounded border border-slate-800/80 text-slate-300">{protocol.timing}</p>
                            <p className="bg-slate-900/60 p-2 rounded border border-slate-800/80 text-slate-300 mt-2">{protocol.dosage}</p>
                          </div>
                        </div>
                        <div className="bg-amber-950/20 border border-amber-900/40 rounded p-3 mt-3 text-[11px] text-amber-200">
                          <strong>Safety notes:</strong> {protocol.safetyNotes.join(" ")}
                        </div>
                        <p className="text-[11px] font-mono text-slate-500 mt-3">Source: {protocol.sourceExcerpt}</p>
                      </div>
                    ))}
                  </div>
                )}
